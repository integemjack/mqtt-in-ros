...
class MqttToRosBridge(Bridge):
    ...
    def _callback_mqtt(self, client: mqtt.Client, userdata: Dict, mqtt_msg: mqtt.MQTTMessage):
        ...
        if mqtt_msg.payload == b'start':
            cmd = "roslaunch ros_deep_learning detectnet.ros1.launch input:=csi://0 output:=display://0"
            rospy.loginfo(cmd)
            # self.proc = subprocess.call(cmd, shell=True, stdout=rospy.loginfo, stderr=rospy.loginfo, bufsize=-1) #, preexec_fn=os.setsid)
            print(os.getpid())
            os.system(cmd)
